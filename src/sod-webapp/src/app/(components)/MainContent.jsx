  ...
  async function handleAnalyzeImage() {
    ...
    try {
      ...
      window.location.href =
        "/results?iaResult=" +
        encodeURIComponent(JSON.stringify(aiData.aiResult));
    } catch (err) {
        ...
        <div className="w-full max-w-md bg-white/80 backdrop-blur-sm p-8 md:p-12 rounded shadow-lg space-y-6">
          ...
          {previewUrl && (
            <div className="flex flex-col items-center space-y-4 w-full">
              ...
              <Button
                className="w-full p-4 bg-[#2d608d] text-white text-lg font-medium rounded hover:bg-[#245179] transition-colors"
                onClick={handleAnalyzeImage}
                disabled={uploading || !axis || !floor}
              >
                {uploading
                  ? uploadSuccess
                    ? "Análise concluída!"
